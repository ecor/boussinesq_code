...
polygon_connection_attribute_array *get_connection_array_squares(POLYGONVECTOR *polygons, long boundary, long novalue, LONGMATRIX *pixels, short print){
	 ...
	 * \param polygons - (POLYGONS *) vector of polygons
	 * \param boundary - (long) long value which indetifies the boundary
	 * \param novalue - null value
	 * \param pixels - (LONGTMATRIX *) - matrix of the indices of the pixels
	 * \param print    - (short)
	 *
	 * \author Emanuele Cordano
	 * \date July 2009
	...
	for (r=pixels->nrl;r<=pixels->nrh;r++) {
		for (c=pixels->ncl;c<=pixels->nch;c++){
			l=pixels->element[r][c];
			if (l!=novalue) {
				pca->element[l]=get_connection_squares(polygons->element[l],polygons,boundary,novalue,r,c,pixels,print);
				if (print==1) printf ("Polygons %ld (%ld connections) of %ld !! \n",l,pca->element[l]->connections->nh,pca->nh);
			}
		}
	}
	for(l=pca->nl;l<=pca->nh;l++){
		if (pca->element[l]==NULL) printf("Error: Polygon connections %ld  of %ld were not allocated!! \n",l,pca->nh);
